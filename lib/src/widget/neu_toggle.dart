...
class NeumorphicToggle extends StatelessWidget {
  ...
  Widget _background(BuildContext context) {
    return Neu(
      style: NeuStyle(
          boxShape: NeuBoxShape.roundRect(this.style?.borderRadius ??
              BorderRadius.all(Radius.circular(12))),
          color: this.style?.backgroundColor,
          disableDepth: this.style?.disableDepth,
          depth: _getTrackDepth(context),
          shape: NeuShape.flat,
          border: this.style?.border ?? NeuBorder.none(),
          lightSource: this.style?.lightSource ??
              NeuTheme.currentTheme(context).lightSource),
  ...
  double _getTrackDepth(BuildContext context) {
    ...
    return depth.clamp(Neu.MIN_DEPTH, NeumorphicToggle.MIN_EMBOSS_DEPTH);
  }
