...
class NeumorphicSwitchStyle {
  ...
  final NeuShape? thumbShape;
  final double? thumbDepth;
  final NeuLightSource? lightSource;
  final bool disableDepth;

  final NeuBorder thumbBorder;
  final NeuBorder trackBorder;

  const NeumorphicSwitchStyle({
    this.trackDepth,
    this.thumbShape = NeuShape.concave,
    ...
    this.thumbBorder = const NeuBorder.none(),
    this.trackBorder = const NeuBorder.none(),
  });
...
class NeumorphicSwitch extends StatelessWidget {
  ...
  NeuShape get _getThumbShape {
    return this.style.thumbShape ?? NeuShape.flat;
  }
  ...
  double? _getTrackDepth(double? themeDepth) {
    ...
    return depth.clamp(Neu.MIN_DEPTH, NeumorphicSwitch.MIN_EMBOSS_DEPTH);
  }

  Color _getTrackColor(NeuThemeData theme, bool enabled) {
    if (!enabled) {
